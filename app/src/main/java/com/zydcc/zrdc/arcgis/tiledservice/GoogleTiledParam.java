 ...
 * ========================================
 */
public class GoogleTiledParam extends BaseTiledParam {
    ...
    public GoogleTiledParam(GisMapConfig gisMapConfig) {
        super(gisMapConfig);
    }
    ...
    public String getUrl(int level, int col, int row, BaseTiledType tiledType) {
        ...
        switch (tiledType) {
            case VEC_C:
                url = url.append(".google.cn/vt/lyrs=m@212000000&hl=zh-CN&gl=CN&src=app&x=").append(col).append("&y=").append(row).append("&z=").append(level).append("&s==Galil");
                break;
            case IMG_C:
                url = url.append(".google.cn/vt/lyrs=s@126&hl=zh-CN&gl=CN&src=app&x=").append(col).append("&y=").append(row).append("&z=").append(level).append("&s==Galil");
                break;
            case CVA_C:
            case CIA_C:
                url = url.append(".google.cn/vt/imgtp=png32&lyrs=h@212000000&hl=zh-CN&gl=CN&src=app&x=").append(col).append("&y=").append(row).append("&z=").append(level).append("&s==Galil");
                break;
            default:
                return null;
        }
